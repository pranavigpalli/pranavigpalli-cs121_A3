...
def process_file(file_path, stemmer):
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        return stemmed_tokens, url  # Return both tokens and URL
...
def write_index_to_file(index, file_path):
    ...
    for token, postings in index.items():
        if token in existing_index:
            for doc_id, (url, freq) in postings.items():
                if doc_id in existing_index[token]:
                    existing_index[token][doc_id][1] += freq  # Update frequency
                else:
                    existing_index[token][doc_id] = [url, freq]
        else:
            existing_index[token] = {doc_id: [url, freq] for doc_id, (url, freq) in postings.items()}
        ...
        if folder.is_dir():
            ...
            for file in folder.iterdir():
                ...
                if file.is_file() and file.suffix == '.json':
                    doc_count += 1
                    tokens, url = process_file(file, stemmer)
                    ...
                    for token, freq in token_freq.items():
                        inverted_index[token][file.name] = (url, freq)
                    if doc_count % 5000 == 0:
